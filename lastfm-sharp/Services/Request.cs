...
namespace Lastfm.Services
{
    ...
    {
        private const string ROOT = "http://ws.audioscrobbler.com/2.0/";
        ...
        public Request(string methodName, Session session, RequestParameters parameters)
        {
            ...
            Parameters["api_key"] = Session.APIKey;
        ...
        internal void SignIt()
        {
            ...
            Parameters["api_sig"] = getSignature();
        }
        ...
        private void Delay()
        {
            ...
            {
                LastCallTime = new DateTime?(DateTime.Now);
            }

            if (DateTime.Now.Subtract(LastCallTime.Value) > new TimeSpan(0, 0, 1))
            {
                Thread.Sleep(1000);
            }
        }
        ...
        private void CheckForErrors(XmlDocument document)
        {
            ...
            var status = n.Attributes[0].InnerText;

            if (status == "failed")
            {
                XmlNode err = document.GetElementsByTagName("error")[0];
                var type = (ServiceExceptionType) Convert.ToInt32(err.Attributes[0].InnerText);
                var description = err.InnerText;
