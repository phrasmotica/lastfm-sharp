...
{
    /// <summary>
    /// A classs to be used by a media player, it caches the tracks to a file first then scrobbles
    ...
    public class ScrobbleManager
    {
        ...
        public ScrobbleManager(Connection connection)
        {
            ...
            CacheDir = Directory.GetParent(Environment.GetFolderPath(Environment.SpecialFolder.Personal)).ToString() + "/Documents/lastfm/";
        ...
        /// <summary>
        /// Queues a <see cref="PlayedTrack"/> for scrobbling, then scrobbles it on another thread.
        /// Or at least tries to. The <see cref="PlayedTrack"/> info is cached locally firt, so
        /// there's no fear of losing precious scrobbles.
        /// </summary>
        /// <param name="track">A <see cref="PlayedTrack"/></param>
        public async Task Queue(Entry track)
        {
            // Append the scrobble line to the file
            StreamWriter writer = new StreamWriter(Path.Combine(CacheDir, cacheFileName), true, Encoding.Unicode);
            writer.WriteLine(track.getParameters(Connection.session).Serialize());
            writer.Flush();
            writer.Close();
        ...
        /// <summary>
        /// Submits the cached scrobbles if any. You shouldn't really concern yourself with calling.
        /// It gets called upon each new queued track.
        /// </summary>
